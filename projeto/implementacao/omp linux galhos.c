...
//gcc omp_linux_galhos.c -fopenmp
//./a.out
...
int qts_folhas(struct no *filho, int paralelo){
    ...
    if(paralelo) filho->verificado = omp_get_thread_num();
...
void mostrar_tree(struct no* filho, int* x, int* y){
    ...
    if(filho != NULL){
		...
		gotoyx((*x),(*y));
		SetConsoleTextAttribute(hConsole, 0x70 - filho->verificado);
		if(CHAVES) impressao(filho);
		else printf("O");
		SetConsoleTextAttribute(hConsole, 7);
...
int main(){
    ...
    libera(z);
    printf("Quantidade de Folhas totais: %d, encontradas pelas threads: %d paralelamente em %lf segundos!",z->qtd_folhas,total,paralelo);
    return 0;
}