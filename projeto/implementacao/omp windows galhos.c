#include <stdio.h>
#include <stdlib.h>
#include <windows.h>					//Qual o melhor numero para testar
#include <time.h>
#include <conio.h>
#include <omp.h>
...
#define VER 1						//se 1, nao imprime as informacoes
#define MOSTRAR 1					//mostrar arvore
	#define CHAVES 1						//imprimir os numeros dos nos
		#define TAM_NUM 2						//numero de digitos dessa iteracao
...
int qts_folhas(struct no *filho, int paralelo){
    ...
    if(paralelo) filho->verificado = (omp_get_thread_num() + 1) * 0x10;
...
int main(){
	...
	inicio = omp_get_wtime();
	z->qtd_folhas = qts_folhas(z->raiz,0);
	fim = omp_get_wtime();
	sequencial = fim - inicio;
	if(!VER) printf("Sequencialmente, %d folhas em %lf segundos!\n",z->qtd_folhas,sequencial);
	...
	#pragma omp parallel num_threads(LINHAS) 
	{
		...
		designado = qts_folhas(galhos[omp_get_thread_num()],1);
		...
		if(!VER) printf("Thread %d verificou %d filhos e achou %d folhas!\n",omp_get_thread_num(),galhos[omp_get_thread_num()]->filhos,designado);
	...
	if(MOSTRAR){
    	...
    	fim = omp_get_wtime();
    	printf("\n\n\n\n\n");
    	printf("%lf segundos para imprimir!\n",(fim-inicio));
	} 
	
	
    libera(z);
    printf("Quantidade de Folhas totais: %d, encontradas pelas threads: %d sequencialmente em %lf segundos!",z->qtd_folhas,total,paralelo);
    return 0;
}